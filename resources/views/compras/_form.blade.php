...
<div class="row">
    <div class="col-md-4">
        <x-adminlte-input name="costo_total" type="number" label="Costo Total ($)" placeholder="Ej: 50.00"
            value="{{ old('costo_total', $compra->costo_total ?? '') }}" min="0" step="0.01" required />
    </div>

    <div class="col-md-4">
        <x-adminlte-input name="cantidad" type="number" label="Cantidad Comprada" placeholder="Ej: 2"
            value="{{ old('cantidad', $compra->cantidad ?? 1) }}" min="1" step="1" required />
    </div>
    ...
    <div class="col-md-4">
        <x-adminlte-select name="tipo_compra" label="Tipo de Compra" required>
            ...
            <option value="Herramienta" {{ $tipo == 'Herramienta' ? 'selected' : '' }}>Herramienta (Genera Alerta)
            </option>
...
<div class="text-right mt-4">
    ...
    {{-- Condicional para mostrar botón según sea creación o edición --}}
    @if (isset($compra))
        @can('compras.update')
