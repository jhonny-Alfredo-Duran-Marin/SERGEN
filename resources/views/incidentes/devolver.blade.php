                        ...
                        <table class="table table-bordered mb-0">
                            ...
                            <tbody>
                                @foreach ($incidente->items as $it)
                                    @php $pivId = $it->pivot->id; @endphp
                                    <tr>
                                        <td>
                                            <strong>{{ $it->codigo }}</strong> ({{ $it->pivot->estado_item }})
                                            <input type="hidden" name="items[{{ $pivId }}][item_id]"
                                                value="{{ $it->id }}">
                                            {{-- Enviamos 'Dañado' o 'Perdido' con la capitalización correcta para la BD --}}
                                            <input type="hidden" name="items[{{ $pivId }}][tipo]"
                                                value="{{ $it->tipo_mapeado }}">
                                        </td>
                                        ...
                                        <td>
                                            <input type="number" name="items[{{ $pivId }}][cantidad_devuelta]"
                                                class="form-control" min="0" max="{{ $it->pendiente_real }}"
                                                value="0">
                                        </td>
                                        <td>
                                            <select name="items[{{ $pivId }}][resultado]" class="form-control">
                                                <option value="DEVUELTO_OK">DEVUELTO_OK (Vuelve a Stock)</option>
